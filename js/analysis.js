...
const Analysis = (() => {
  ...
  // Sucursales (puntos de venta) con menos visitas registradas. Se calcula
  // sobre la pestaña de quiebres porque es la que releva absolutamente todos
  // los puntos de venta del mes — "días distintos con al menos una visita"
  // es más representativo que contar filas, porque un PDV grande genera
  // muchas filas por visita (una por SKU) y eso lo haría parecer más
  // visitado de lo que realmente es.
  function visitasPorPDV(rows){
    const map = new Map();
    rows.forEach(r => {
      const key = r.puntoVenta || '(Sin dato)';
      if (!map.has(key)) map.set(key, { key, cadena: r.cadena, dias: new Set(), filas: 0 });
      const g = map.get(key);
      if (r.fecha) g.dias.add(r.fecha.toDateString());
      g.filas += 1;
    });
    return Array.from(map.values()).map(g => ({
      key: g.key, cadena: g.cadena, diasVisitados: g.dias.size, relevamientos: g.filas,
    })).sort((a, b) => a.diasVisitados - b.diasVisitados);
  }

  function sugerirUmbralVisitas(visitas){
    const conteos = visitas.map(v => v.diasVisitados);
    const mediana = medianOf(conteos);
    // Igual que la cobertura de PDV: son números chicos, así que se usa la
    // mediana entera en vez de la mitad.
    return Math.max(2, Math.round(mediana));
  }

  // ---- Indicadores genéricos (disponibilidad / cumplimiento por ítem) ----
  // Agrupa filas que ya traen un flag 0/1 (ej. disponibleFlag, cumpleFlag)
  // por una dimensión, y devuelve el % de "1" (Si) por grupo.
  function groupFlagPct(rows, dimKey, flagKey){
    const map = new Map();
    rows.forEach(r => {
      const key = r[dimKey] || '(Sin dato)';
      if (!map.has(key)) map.set(key, { key, total: 0, positivos: 0 });
      const g = map.get(key);
      g.total += 1;
      g.positivos += r[flagKey];
    });
    return Array.from(map.values()).map(g => ({
      ...g, pct: g.total ? g.positivos / g.total : 0,
    }));
  }

  // Promedio de un campo numérico (ej. shareUL) por dimensión
  function groupAverage(rows, dimKey, valueKey){
    const map = new Map();
    rows.forEach(r => {
      if (r[valueKey] == null) return;
      const key = r[dimKey] || '(Sin dato)';
      if (!map.has(key)) map.set(key, { key, suma: 0, n: 0 });
      const g = map.get(key);
      g.suma += r[valueKey]; g.n += 1;
    });
    return Array.from(map.values()).map(g => ({ key: g.key, promedio: g.n ? g.suma / g.n : null, n: g.n }));
  }

  // Suma de un campo numérico (ej. cantidad de espacios) por dimensión
  function groupSum(rows, dimKey, valueKey){
    const map = new Map();
    rows.forEach(r => {
      const key = r[dimKey] || '(Sin dato)';
      if (!map.has(key)) map.set(key, { key, total: 0 });
      map.get(key).total += (r[valueKey] || 0);
    });
  ...
  function colorForPct(pct, domainMin = 0, domainMax = 1, invert = false){
  ...
  return {
    ...
    visitasPorPDV, sugerirUmbralVisitas,
    groupFlagPct, groupAverage, groupSum,
